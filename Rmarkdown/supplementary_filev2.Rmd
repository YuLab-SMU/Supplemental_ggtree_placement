...
p2 <- ggtree::ggtree(tree1, layout = "circular", branch.length = "none", aes(color = nplace, size = nplace)) +
    colorspace::scale_color_continuous_sequential(palette = "Sunset", limits = c(1, 55)) +
...
# Plot the subsetted tree using a rectangular layout, colored by likelihood weight ratio
p3 <- ggtree::ggtree(tree3, layout = "rect", aes(color = like_weight_ratio), size = 1) +
    colorspace::scale_color_continuous_sequential(palette = "Sunset", limits = c(0, 1)) +
...
tree3@extraInfo[, 5] <- tree3@extraInfo[, 5] |> 
    as.vector() |> 
    unlist() |> 
    as.numeric() |> 
    round(digits = 4) |> 
...
# Plot the subsetted tree with additional labels and adjustments
p4 <- ggtree(tree3, layout = "rect", size = 1) +
    colorspace::scale_color_continuous_sequential(palette = "Sunset", limits = c(0, 1)) +
    geom_tiplab(size = 3, color = "black", align = TRUE, offset = 0) + 
    xlim(0, 2) +
    geom_label2(aes(x = branch, label = like_weight_ratio), color = 'blue', fill = "green", size = 2.5)

# Combine the plots p2, p3, and p4 into a single multi-panel plot
aplot::plot_list(p2, p3, p4, ncol = 2, nrow = 2,
          tag_levels = list(c('(a)', '(b)', '(c)')), 
          tag_size = 20, design = "ABBB\nABBB\nCCCC\nCCCC")
...
p2 <- ggtree::ggtree(tree5, layout = "circular") +
    ...
    colorspace::scale_color_continuous_sequential(palette = "Sunset", limits = c(0.000001, 0.2)) +  # Set color scale
    scale_size_continuous(range = c(0, 1.5), guide = guide_legend(reverse = TRUE)) +  # Scale size of branches
